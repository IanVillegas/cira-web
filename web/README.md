...
- El backend [`server`](../server) corriendo en paralelo para datos reales
  (SQLite). Sin él, la mayoría de pantallas muestran un estado de "sin
  conexión" en vez de datos.

## Uso

```bash
# 1. Backend (en otra terminal, desde server/)
...
- `src/lib/api.ts` — cliente HTTP hacia el backend (`server/`).
